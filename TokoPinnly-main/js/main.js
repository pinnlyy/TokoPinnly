...
async function initProductsPage() {
  const grid = qs("#productGrid"), search = qs("#searchInput"), categoryChips = qs("#categoryChips"), sortChips = qs("#sortChips"), counter = qs("#productCounter");
  if (renderConfigWarning(grid)) return;

  let selectedCategory = getParam("category") || "all";
  let selectedSort = "newest";

  const setActiveChip = (wrap, selector, value) => {
    if (!wrap) return;
    qsa(selector, wrap).forEach((btn) => {
      const current = btn.dataset.category || btn.dataset.sort || "all";
  ...
  try {
    const [products, categories] = await Promise.all([fetchProducts(), fetchCategories()]);
    if (selectedCategory !== "all" && !categories.some((c) => c.name === selectedCategory)) selectedCategory = "all";

    if (categoryChips) {
      categoryChips.innerHTML = `<button class="filter-chip" type="button" data-category="all">Semua</button>` +
        categories.map((c) => `<button class="filter-chip" type="button" data-category="${escapeHtml(c.name)}">${escapeHtml(c.name)}</button>`).join("");
      setActiveChip(categoryChips, "[data-category]", selectedCategory);
    }
    ...
    const update = () => {
      ...
      renderProductCards(grid, res);
      if (counter) counter.textContent = `${res.length} produk ditemukan`;
    };
    ...
    sortChips?.addEventListener("click", (e) => {
      ...
      if (!btn) return;
      selectedSort = btn.dataset.sort || "newest";
      setActiveChip(sortChips, "[data-sort]", selectedSort);
      update();
    });
    ...
    console.error(e);
    grid.innerHTML = `<div class="error-state glass-card"><h3>Gagal memuat katalog</h3><p>${escapeHtml(e.message)}</p></div>`;
  }
}
